...
/**
 *	Generic Ehernet connection class.
 *
 *	This class present generic Ethernet access interface.
 *	Childs of this class can implement different methods of access to
 *	Ehternet hadware of host PC (e.g. direct access or over tunnel device).
 */
class EthDevice: public PacketDevice {
public:
	virtual ~EthDevice() {}
 ...
 */
class EthTunDevice: public EthDevice {
public:
	virtual ~EthTunDevice() {}
};

/* system-dependent (implementation in $MYSYSTEM/ *.cc) */
extern EthDevice *createEthernetTunnel();

/**
 *	Pcap-based Ethernet access class.
 *
 *	This class presents direct access (as opposed to tunneling) to
 *	ethernet controller over PCAP interface.
 */
class EthPcapDevice: public EthDevice {
public:
	virtual ~EthPcapDevice() {}
};

/**
 *	Create an pcap-conection to real a phisycal Ethernet card.
 */
extern EthDevice *createEthernetPcap(char *interfaceName);

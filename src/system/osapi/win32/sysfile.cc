...
int sys_truncate(const char *filename, FileOfs ofs)
{
	...
	if (SetFilePointer(hfile, ofs, &highbits, FILE_BEGIN)==0xffffffff) {
		CloseHandle(hfile);
		return EIO;
	}
...
int sys_truncate_sys_file(SYS_FILE *file, FileOfs ofs)
{
	LONG highbits = ofs >> 32;
	
	if (SetFilePointer(*(HANDLE *)file, ofs, &highbits, FILE_BEGIN)==0xffffffff) {
		return EIO;
	}

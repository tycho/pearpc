...
void txDPD0(DPD0 *dpd)
{
	...
	uint framePrefix = mEth->getWriteFramePrefix();
...
public:
_3c90x_NIC(EthDevice *aEth, const byte *mac)
...
void handleRxQueue()
{
	while (1) {
		...
		if (mRxPacketSize) {
			...
			mRxPacketSize = mEth->recvPacket(mRxPacket, sizeof mRxPacket);
			if (mRxEnabled && (mRxPacketSize > sizeof(EthFrameII))) {
				...
				if (!passesRxFilter(mRxPacket, mRxPacketSize)) {
					IO_3C90X_TRACE("Eth: %d bytes received. But they don't pass the filter.\n", mRxPacketSize);
					mRxPacketSize = 0;
				} else {
					IO_3C90X_TRACE("Eth: %d bytes received.\n", mRxPacketSize);
				}
			}  else {
				// don't block the system in case of (repeated) error(s)
				mRxPacketSize = 0;
...
bool _3c90x_type_pcap = false;

#define _3C90X_KEY_INSTALLED	"pci_3c90x_installed"
#define _3C90X_KEY_MAC		"pci_3c90x_mac"
#define _3C90X_KEY_TYPE		"pci_3c90x_type"
#define _3C90X_KEY_INTERFACE	"pci_3c90x_interface"

#define _3C90X_KEY_TYPE_TAP	"tap"
#define _3C90X_KEY_TYPE_PCAP	"pcap"
...
void _3c90x_init()
{
	if (gConfig->getConfigInt(_3C90X_KEY_INSTALLED)) {
		...
		_3c90x_type_pcap = false;
		if (gConfig->haveKey(_3C90X_KEY_TYPE)) {
			String type;
			gConfig->getConfigString(_3C90X_KEY_TYPE, type);
			if (type == _3C90X_KEY_TYPE_TAP) {
				_3c90x_type_pcap = false;
			} else if (type == _3C90X_KEY_TYPE_PCAP) {
				_3c90x_type_pcap = true;
			} else {
				IO_3C90X_ERR("error in config key %s:"
					"value must be '%s' or '%s' but not '%s'\n",
					_3C90X_KEY_TYPE,
					_3C90X_KEY_TYPE_TAP,
					_3C90X_KEY_TYPE_PCAP,
					type.contentChar());
			}
		}
		EthDevice *eth = NULL;
		if (_3c90x_type_pcap) {
			String _3c90x_interface;
			if (!gConfig->haveKey(_3C90X_KEY_INTERFACE)) {
				IO_3C90X_ERR("error: no '%s' key specified "
					"for PCAP device\n",
					_3C90X_KEY_INTERFACE);
				exit(1);
			}
			gConfig->getConfigString(_3C90X_KEY_INTERFACE, _3c90x_interface);
			eth = createEthernetPcap(_3c90x_interface.contentChar());
			if (!eth) {
				IO_3C90X_ERR("Couldn't create PCAP ethernet bridge\n");
				exit(1);
			}
		} else {
		...
		printf("\n");
#endif
		_3c90x_NIC *MyNIC = new _3c90x_NIC(eth, mac);
...
void _3c90x_init_config()
{
	...
	gConfig->acceptConfigEntryString(_3C90X_KEY_TYPE, _3C90X_KEY_TYPE_TAP);
	gConfig->acceptConfigEntryString(_3C90X_KEY_INTERFACE, "");
}

...
{
	static uint64 time_to = 0;
	const uint timeout_msec = 200;

	if(time_to == 0) {
		time_to = sys_get_hiresclk_ticks_per_second() * timeout_msec / 1000;
	}

	uint64 time_end = sys_get_hiresclk_ticks() + time_to;
//	ht_printf("process  %d\n", ev.key.pressed);
...
void cuda_init()
{
	ticks_per_sec = 1000ULL * sys_get_hiresclk_ticks_per_second();

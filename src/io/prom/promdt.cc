...
/*	//
	...
	IDEConfig *ic[IDE_CHANNEL_MAX * IDE_DISK_MAX];
	for (int CHANNEL = 0; CHANNEL < IDE_CHANNEL_MAX; CHANNEL++) {
		char nodename[255];
		sprintf(nodename, "pci-ata@%d", CHANNEL + 1);
	
		PromNode *pci_ata = new PromNode(nodename);
		bridge->addNode(pci_ata);
		bridge->addNodeShort("pci-ata", nodename);
		...
		ic[CHANNEL * IDE_DISK_MAX] = ide_get_config(CHANNEL * IDE_DISK_MAX);
		ic[CHANNEL * IDE_DISK_MAX + 1] = ide_get_config(CHANNEL * IDE_DISK_MAX + 1);
		if (ic[CHANNEL * IDE_DISK_MAX]->installed || ic[CHANNEL * IDE_DISK_MAX + 1]->installed) {
			...
			if (ic[CHANNEL * IDE_DISK_MAX]->installed) {
				char diskname[255];
				char shortdiskname[255];

				sprintf(diskname, "disk%d@%d", CHANNEL * IDE_DISK_MAX, CHANNEL * IDE_DISK_MAX);
				sprintf(shortdiskname, "disk%d", CHANNEL * IDE_DISK_MAX);

				PromNode *disk1 = new PromNodeDisk(diskname, CHANNEL * IDE_DISK_MAX);
				ata4->addNode(disk1);
				ata4->addNodeShort("disk", diskname);
				ata4->addNodeShort(shortdiskname, diskname);
				disk1->addProp(new PromPropInt("device-id", CHANNEL * IDE_DISK_MAX));
			...
			if (ic[CHANNEL * IDE_DISK_MAX + 1]->installed) {
				char diskname[255];
				char shortdiskname[255];

				sprintf(diskname, "disk%d@%d", CHANNEL * IDE_DISK_MAX + 1, CHANNEL * IDE_DISK_MAX + 1);
				sprintf(shortdiskname, "disk%d", CHANNEL * IDE_DISK_MAX + 1);

				PromNode *disk2 = new PromNodeDisk(diskname, CHANNEL * IDE_DISK_MAX + 1);
				ata4->addNode(disk2);
				ata4->addNodeShort(shortdiskname, diskname);
				disk2->addProp(new PromPropInt("device-id", CHANNEL * IDE_DISK_MAX + 1));
...
*/	
	...
	for (int i=0; i<(IDE_CHANNEL_MAX * IDE_DISK_MAX); i++) {
		if (ic[i]->installed) {
...
//				location.assignFormat("/pci/pci-bridge/pci-ata@%d/ata-4/disk%d@%d", i / 2 + 1, i, i);
				aliases->addProp(new PromPropString(alias.contentChar(), location.contentChar()));
				ht_printf("[IO/PROM] Add alias %6s -> %s\n", alias.contentChar(), location.contentChar());
				if (!hdcount) {

...
static void read_partitions_of_device(Container &brs, bool only_bootable, const char *boot_device)
{
...
static void read_partitions(Container &brs, bool only_bootable)
{
	...
	for(int i = 0; i < (IDE_CHANNEL_MAX * IDE_DISK_MAX); i++) {
		sprintf(boot_device, "cdrom%d", i);
		read_partitions_of_device(brs, only_bootable, boot_device);
	}

	for(int i = 0; i < (IDE_CHANNEL_MAX * IDE_DISK_MAX); i++) {
		sprintf(boot_device, "disk%d", i);
		read_partitions_of_device(brs, only_bootable, boot_device);
	}

/*	char *boot_devices[] = {
#if IDE_CHANNEL_MAX >= 1
		"cdrom0", "cdrom1",
#if IDE_CHANNEL_MAX >= 2
		"cdrom2", "cdrom3",
#if IDE_CHANNEL_MAX >= 3
		"cdrom4", "cdrom5",
#if IDE_CHANNEL_MAX >= 4
		"cdrom6", "cdrom7",
#if IDE_CHANNEL_MAX >= 5
		#error IDE_CHANNEL_MAX is too big!!!
#endif
#endif
#endif
#endif
